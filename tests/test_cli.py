...
class ConvexLanceCliTest(unittest.TestCase):
    ...
    def test_create_indexes_for_generated_columns(self):
        ...
        created, skipped = create_indexes_for_columns(conn, "claims", "'s3://bucket/tables/claims.lance'", generated_index_columns())

        self.assertEqual(created, 3)
        self.assertEqual(skipped, [])
        self.assertIn("CREATE INDEX claims___id_ts_idx ON 's3://bucket/tables/claims.lance' (__id_ts) USING BTREE", conn.statements)
        self.assertIn("CREATE INDEX claims___status_id_idx ON 's3://bucket/tables/claims.lance' (__status_id) USING BTREE", conn.statements)
        self.assertIn("CREATE INDEX claims___status_idx ON 's3://bucket/tables/claims.lance' (__status) USING BITMAP", conn.statements)
